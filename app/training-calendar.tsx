...
const plans:Record<DayKey,{label:string;short:string;color:string;exercises:Exercise[]}> = {
  ...
  day2:{label:'DAY 2 · 胸＋手臂＋腹',short:'推',color:'orange',exercises:[
    ...
    {name:'啞鈴彎舉 / 牧師椅彎舉',sets:'3 × 8–12',base:7,step:1,every:3,note:'二選一。重量欄：啞鈴記單手重量；牧師椅記器材顯示重量。'},
...
const cues:Record<string,string> = {
  ...
  '啞鈴彎舉 / 牧師椅彎舉':'啞鈴：手肘貼近身體、手腕保持直；牧師椅：上臂貼墊、肘關節對準轉軸，底端不反彈。',
...
function localDateKey(){const d=new Date();return dateKey(new Date(d.getFullYear(),d.getMonth(),d.getDate(),12));}
function calendarIndexForKey(value:string){const [year,month,day]=value.split('-').map(Number);return Math.round((Date.UTC(year,month-1,day)-Date.UTC(2026,7,28))/86400000);}
...
export default function TrainingCalendar(){
  ...
  useEffect(()=>{
    const syncToday=()=>setTodayKey(localDateKey());
    syncToday();
    const timer=window.setInterval(syncToday,60000);
    return()=>window.clearInterval(timer);
  },[]);
  useEffect(()=>{try{const saved=localStorage.getItem('rick-training-overrides');if(!saved)return;const parsed:Overrides=JSON.parse(saved);const combined='啞鈴彎舉 / 牧師椅彎舉';if(!parsed[combined]){const preferred=localStorage.getItem('rick-training-biceps-choice')==='preacher'?'牧師椅彎舉':'啞鈴彎舉';const legacy=parsed[preferred]||parsed['啞鈴彎舉']||parsed['牧師椅彎舉'];if(legacy){parsed[combined]=legacy;localStorage.setItem('rick-training-overrides',JSON.stringify(parsed));}}setOverrides(parsed);}catch{}},[]);
  useEffect(()=>{
    if(!todayKey)return;
    const index=dates.findIndex(entry=>dateKey(entry.date)===todayKey);
    if(index<0)return;
    setSelected(index);
    setMonthCursor(Math.max(0,monthOptions.findIndex(month=>month.year===dates[index].date.getFullYear()&&month.month===dates[index].date.getMonth())));
    setEditing(false);
    setDraft({});
